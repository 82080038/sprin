-- Production Features Migration for SPRIN Application
-- Created: 2026-04-11
...
-- Add wellness tracking to personil table (skip if columns already exist)
-- ALTER TABLE personil 
-- ADD COLUMN wellness_score INT DEFAULT 100 COMMENT 'Skor kesehatan 0-100',
-- ADD COLUMN max_weekly_hours DECIMAL(5,2) DEFAULT 40.00 COMMENT 'Maksimal jam kerja per minggu',
-- ADD COLUMN fatigue_level ENUM('low','medium','high','critical') DEFAULT 'low',
-- ADD COLUMN last_fatigue_check DATE NULL;

-- Add index on nrp for foreign key constraints (skip if already exists)
-- ALTER TABLE personil ADD INDEX idx_nrp (nrp);
...
CREATE TABLE IF NOT EXISTS fatigue_tracking (
    ...
    FOREIGN KEY (personil_id) REFERENCES personil(nrp) ON DELETE CASCADE,
...
-- Add fatigue tracking to schedules (skip if columns already exist)
-- ALTER TABLE schedules 
-- ADD COLUMN fatigue_risk ENUM('low','medium','high') DEFAULT 'low',
-- ADD COLUMN consecutive_days INT DEFAULT 1 COMMENT 'Hari kerja berturut-turut',
-- ADD COLUMN weekly_hours DECIMAL(5,2) DEFAULT 0 COMMENT 'Total jam kerja minggu ini';
...
CREATE TABLE IF NOT EXISTS emergency_tasks (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (assigned_to) REFERENCES personil(nrp) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS task_conflicts (
    ...
    FOREIGN KEY (emergency_task_id) REFERENCES emergency_tasks(id) ON DELETE CASCADE,
...
ALTER TABLE jabatan 
ADD COLUMN required_certifications JSON NULL COMMENT 'List of required certifications',
ADD COLUMN certification_check BOOLEAN DEFAULT FALSE COMMENT 'Check certifications before assignment';
...
-- Add overtime tracking to schedules
ALTER TABLE schedules 
ADD COLUMN overtime_hours DECIMAL(4,2) DEFAULT 0,
ADD COLUMN overtime_rate ENUM('regular','holiday','weekend','emergency') DEFAULT 'regular',
ADD COLUMN overtime_approved BOOLEAN DEFAULT FALSE,
ADD COLUMN overtime_approved_by VARCHAR(50);
...
CREATE TABLE IF NOT EXISTS scheduling_patterns (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (bagian_id) REFERENCES bagian(id) ON DELETE CASCADE,
    FOREIGN KEY (unsur_id) REFERENCES unsur(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS equipment (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (current_assignment) REFERENCES personil(nrp) ON DELETE SET NULL,
...
-- Add equipment requirements to operations
ALTER TABLE operations 
ADD COLUMN required_equipment JSON NULL COMMENT 'List of required equipment',
ADD COLUMN equipment_assigned BOOLEAN DEFAULT FALSE;
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (target_personil) REFERENCES personil(nrp) ON DELETE CASCADE,
...
-- Create system settings table for production features
...
('fatigue_max_weekly_hours', '40', 'integer', 'Maximum weekly working hours', 'fatigue'),
('fatigue_min_rest_hours', '12', 'integer', 'Minimum rest hours between shifts', 'fatigue'),
('fatigue_consecutive_days_limit', '7', 'integer', 'Maximum consecutive working days', 'fatigue'),
('certification_expiry_warning_days', '30', 'integer', 'Days before expiry to send warning', 'certification'),
('overtime_auto_approval_limit', '2', 'decimal', 'Auto-approve overtime under this limit', 'overtime'),
('emergency_task_auto_assign', 'false', 'boolean', 'Auto-assign emergency tasks', 'emergency'),
('recall_response_timeout_minutes', '30', 'integer', 'Timeout for recall response in minutes', 'recall'),
('equipment_maintenance_warning_days', '7', 'integer', 'Days before maintenance to send warning', 'equipment')
...
ALTER TABLE schedules ADD INDEX idx_fatigue_risk (fatigue_risk, shift_date);
ALTER TABLE schedules ADD INDEX idx_personil_date (personil_id, shift_date);
ALTER TABLE personil ADD INDEX idx_wellness_score (wellness_score);
ALTER TABLE operations ADD INDEX idx_status_dates (status, operation_date, operation_date_end);

-- Add full-text search indexes
ALTER TABLE personil ADD FULLTEXT(nama, nrp);
ALTER TABLE operations ADD FULLTEXT(operation_name, location);
ALTER TABLE emergency_tasks ADD FULLTEXT(task_name, description);
