-- Notification Statistics and Monitoring Tables
-- Created: 2026-04-11
...
SELECT 
    ...
    AVG(CASE WHEN ndl.delivery_time_ms IS NOT NULL THEN ndl.delivery_time_ms END) as avg_delivery_time_ms
...
BEGIN
    ...
    -- Delete mobile notification logs older than 30 days
...
-- Create stored procedure for notification queue processing
DELIMITER //
CREATE PROCEDURE IF NOT EXISTS ProcessNotificationQueue()
BEGIN
    ...
    -- Mark failed notifications that exceeded max attempts
    UPDATE notification_queue 
    SET status = 'failed', error_message = 'Max attempts exceeded'
    WHERE status = 'processing' 
    AND attempts >= max_attempts;
    
    SELECT processed_count as notifications_processed;
END //
DELIMITER ;
