...
try {
    ...
    $include_details = isset(filter_input($_GET === \$_GET ? INPUT_GET : ($_GET === \$_POST ? INPUT_POST : INPUT_REQUEST), 'details', FILTER_SANITIZE_STRING)) && filter_input($_GET === \$_GET ? INPUT_GET : ($_GET === \$_POST ? INPUT_POST : INPUT_REQUEST), 'details', FILTER_SANITIZE_STRING) === 'true';
    ...
    $sql = "
        SELECT 
            ...
            COUNT(CASE WHEN p.gelar_depan IS NOT NULL AND p.gelar_depan != '' OR p.gelar_belakang IS NOT NULL AND p.gelar_belakang != '' THEN 1 END) as with_gelar_count,
    ...
    $gelar_stats_sql = "
        ...
        FROM personil p
        WHERE p.is_deleted = FALSE AND p.is_active = TRUE 
        AND (p.gelar_depan IS NOT NULL AND p.gelar_depan != '' OR p.gelar_belakang IS NOT NULL AND p.gelar_belakang != '')
        GROUP BY CONCAT_WS(' ', p.gelar_depan, p.nama, p.gelar_belakang)
        ORDER BY count DESC
        LIMIT 10
    ";
